...
class Presenter(models.Model):
    ...
    credentials = HTMLField(
        help_text='Enter the current designation of the speaker, and past work experience', db_column='Credentials', blank=True)
    profile = HTMLField(
        help_text='Enter the qualification and other details', db_column='Profile')
...
class Lecture(models.Model):
    ...
    photo = models.ImageField(upload_to=(
        'Speakerphotos'), max_length=50, blank=True)
...
class Feedback(models.Model):
    ...
    poster = models.ImageField(upload_to=(
        'posters'), max_length=150, blank=True)
...
class SignUp(models.Model):
    ...
    discipline_first = models.CharField(
        max_length=50, blank=True,  default='Other')
    discipline_dual = models.CharField(
        max_length=50, blank=True,  default='Not Applicable')
    phone = models.CharField(max_length=13, blank=True, default='+91')
    biography = models.TextField('Short Biography', blank=True)
    lect_topics = models.TextField(
        'What topics would you like to give the lecture on?*')
    talk_discipline = models.CharField(
        'Select the Discipline that matches the topics you wish to deliver a talk on', max_length=50, blank=True,  default='Interdisciplinary')
...
class Atmos(models.Model):
    ...
    #presenter = models.ForeignKey(Presenter, on_delete=models.PROTECT)
    photo = models.ImageField(upload_to=(
        "AtmosPhotos"), max_length=50, blank=True)
    discussion = models.TextField(blank=True)
    #feedback = models.FileField(upload_to = ("./feedback"), blank = True)
    allowed = models.BooleanField('Display on site', default=True)
...
class AIC_Company(models.Model):
    ...
    logo = models.ImageField(upload_to=(
        "./AIC_CompanyPhotos"), max_length=50, blank=True)
    problem_statement_details = models.TextField(blank=True)
    problem_statement_file = models.FileField(
        upload_to=("./Problem_Statements"), blank=True)
    allowed = models.BooleanField('Display on site', default=True)
...
class AIC_Solution(models.Model):
    team_name = models.CharField(
        'Team Name* (Create any single word teamname.)   ', max_length=100, blank=False)
    project_name = models.CharField(
        'Project Name* (Preferably Problem Name.)   ', max_length=100, blank=False)
    member_one_name = models.CharField(
        'Member One Name*   ', max_length=100, blank=False)
    member_one_email = models.EmailField('Member One Email*   ', blank=True)
    member_two_name = models.CharField(
        'Member Two Name   ', max_length=100, blank=True)
    member_two_email = models.EmailField('Member Two Email   ', blank=True)
    member_three_name = models.CharField(
        'Member Three Name   ', max_length=100, blank=True)
    member_three_email = models.EmailField('Member Three Email   ', blank=True)
    member_four_name = models.CharField(
        'Member Four Name   ', max_length=100, blank=True)
    member_four_email = models.EmailField('Member Four Email   ', blank=True)
    member_five_name = models.CharField(
        'Member Five Name   ', max_length=100, blank=True)
    member_five_email = models.EmailField('Member Five Email   ', blank=True)
    description = models.TextField(
        'Briefly describe your solution (at max 500 characters.)    ', blank=True)
    company_discipline = models.CharField(
        'Choose the track under which you are applying for the company*', max_length=50, blank=False, default='Not Applicable')
