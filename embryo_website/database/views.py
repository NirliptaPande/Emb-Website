...
def get_all_lectures(search_item=None):
    if search_item:
        lecture_topic = Lecture.objects.filter(
            topic__icontains=search_item, allowed=True)
...
def upcoming_lectures():
    upcoming_lectures = Lecture.objectsfilter(
        date__gte=datetime.datetime.now(), allowed=True)
    return upcoming_lectures


def recent_lectures():
    all_lectures = Lecture.objects.filter(
        date__lt=datetime.datetime.now(), allowed=True)
...
def get_companies(discipline_id):
    discipline_companies = AIC_Company.objects.filter(
        discipline=discipline_id, allowed=True)
