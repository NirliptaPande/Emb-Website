...
def writeup(request, name='home'):
    ...
    return render(request, 'content.html', dictionary)
...
def eventregister(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            body = make_url(request.get_host(), name,
                            bits_id, phone, bits_email)
            success = send_mail(subject, body, "embryoclub@gmail.com", [
                                bits_email, ], fail_silently=False)
...
def atmos_list(request, page_id=0):
    ...
    dictionary['all_atmos_lectures'] = get_all_atmos_lectures()[
        page_id:page_id+5]
    dictionary['all_discipline'] = Discipline.objects.all()
